...
class TestArgumentParser:
    ...
    def test_parse_args_boolean_flags_negative(self):
        """Test parse_args with negative boolean flags"""
        # Arrange
        args = [
            "config.json",
            "--no-skip-input-sets",
        ...
        # Assert
        assert result.no_skip_input_sets is True
        assert result.no_skip_triggers is True
        assert result.no_skip_templates is True
        assert result.no_update_existing is True
        assert result.no_output_json is True
        assert result.no_output_color is True
    ...
    def test_parse_args_complex_combination(self):
        ...
        # Assert
        assert result.config_file == "my-config.json"
        assert result.source_url == "https://source.harness.io"
        assert result.source_api_key == "source-key"
        assert result.dest_url == "https://dest.harness.io"
        assert result.dest_api_key == "dest-key"
    ...
    def test_parse_args_invalid_argument(self):
        ...
        # Arrange
        args = ["config.json", "--invalid-argument", "value"]
    ...
    def test_parse_args_missing_config_file(self):
        """Test parse_args without required config file argument"""
        # Arrange
        args = ["--debug"]
    ...
    def test_parse_args_url_validation_format(self):
        ...
        for url in test_cases:
            args = ["config.json", "--source-url", url]
    ...
    def test_parse_args_boolean_flag_defaults(self):
        ...
        # Assert
        assert result.skip_input_sets is None
        assert result.no_skip_input_sets is None
        assert result.skip_triggers is None
        assert result.no_skip_triggers is None
        assert result.skip_templates is None
        assert result.no_skip_templates is None
        assert result.update_existing is None
        assert result.no_update_existing is None
        assert result.output_json is None
        assert result.no_output_json is None
        assert result.output_color is None
        assert result.no_output_color is None
        assert result.save_config is None
    ...
    def test_parse_args_conflicting_boolean_flags(self):
        """Test parse_args with conflicting boolean flags (both positive and negative)"""
        # Arrange
        args = [
            "config.json",
            "--skip-input-sets",
        ...
        # Assert
        # Both flags should be set to True (last one wins in argparse)
        assert result.skip_input_sets is True
        assert result.no_skip_input_sets is True
    ...
    def test_parse_args_api_key_handling(self):
        ...
        for api_key in api_keys:
            args = ["config.json", "--source-api-key", api_key]
